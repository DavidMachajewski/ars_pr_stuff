...
def parse_grid(path):
    ...
    return final_image, digits


def extract_sudoku(image_path):
    final_image, dig = parse_grid(image_path)
    return final_image, dig


# if __name__ == '__main__':
#	main()
"""
image, digits = extract_sudoku("C:/Users/Natalia/Desktop/SudokuBilder/full_sudokus/data/002.jpeg")
test = show_image(image)

print(digits[0][0])

counter = 0
for dg in digits:
    cv2.imwrite('C:/Users/Natalia/PycharmProjects/ocr/dataset/' + str(counter) + '.jpg', dg)
    counter = counter + 1
counter = 0
"""
labels = [
    3, 11, 15, 6, 14, 18, 12, 9, 17,
    17, 4, 16, 11, 2, 19, 18, 5, 13,
    12, 8, 19, 15, 7, 13, 1, 6, 14,
    9, 16, 12, 3, 11, 4, 7, 18, 15,
    14, 5, 13, 17, 8, 16, 19, 2, 11,
    18, 17, 1, 9, 15, 12, 14, 13, 6,
    16, 2, 7, 18, 3, 11, 15, 4, 19,
    15, 9, 18, 14, 6, 17, 13, 1, 12,
    11, 3, 14, 12, 19, 5, 16, 17, 8
]

def slice_all_sudokus_from_folder(folder, outfolder):
    counter = 0
    for filename in os.listdir(folder):
        image, digits = extract_sudoku(folder + filename)
        for dg in digits:
            cv2.imwrite(outfolder + str(counter) + '.jpg', dg)
            counter = counter + 1

"""
counter = 0
folder = "C:/Users/Natalia/Desktop/SudokuBilder/full_sudokus/data/"
for filename in os.listdir(folder):
    image, digits = extract_sudoku(folder + filename)
    for dg in digits:
        cv2.imwrite('C:/Users/Natalia/PycharmProjects/ocr/dataset/real_with_noise/' + str(counter) + '.jpg', dg)
        counter = counter + 1
"""
...
print(df['labels'].iloc[:6])
"""
pathfile = 'C:/Users/Natalia/Desktop/SudokuBilder/full_sudokus/labels2.txt'
